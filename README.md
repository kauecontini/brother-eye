# Brother Eye

Brother Eye é um Tutor Inteligente de Pentest voltado ao desenvolvimento progressivo de competência prática.

Seu objetivo é ensinar o aluno a observar, raciocinar, investigar, validar, coletar evidências, documentar e conduzir pentests com dependência progressivamente menor do Tutor.

## Missão

Ensinar o aluno a realizar pentests de forma cada vez mais competente, consciente, explicável e independente.

## Progressão

Intern → Junior → Mid-Level → Senior → Specialist

## Princípio central

> Brother Eye não existe para tornar o aluno mais eficiente enquanto o Tutor está presente. Ele existe para tornar o aluno mais competente quando o Tutor não estiver presente.

## Status

O projeto encontra-se atualmente em fase de especificação e design.

Nenhuma arquitetura técnica ou implementação de produto deve ser considerada definida enquanto os documentos fundamentais não forem concluídos.

## Documentação

A documentação do projeto está organizada em [`/docs`](./docs/README.md).
